...
import {
  ...
  normalizeSafeRelatedDomain,
  prepareSelectedRelatedDomainRules
} from "../src/popup/relatedDomains/model";
...
describe("related-domain pure model boundary", () => {
  it("normalizes safe candidates and rejects private or stored-denylisted targets", () => {
    expect(normalizeSafeRelatedDomain("https://www.Example.com/path?q=secret")).toBe("example.com");
    expect(normalizeSafeRelatedDomain("http://192.168.1.10/status")).toBeNull();
    expect(normalizeSafeRelatedDomain("cdn.example.com", ["example.com"])).toBeNull();
  });

  it("preserves candidate order and deduplicates selected save inputs without mutating them", () => {
    const preview = {
      status: "success" as const,
      message: "Preview ready.",
      currentDomain: "example.com",
      collectedHosts: ["cdn.related.example", "static.related.example"],
      candidates: {
        currentDomain: "example.com",
        strongCandidates: [
          {
            domain: "related.example",
            reason: "explicit-related-domain" as const,
            sourceHosts: ["cdn.related.example"],
            sourceHostCount: 1,
        ...
        mediumCandidates: [
          {
            ...
            sourceHosts: ["static.related.example"],
            sourceHostCount: 1,
            suggestedIncludeSubdomains: true,
    ...
    const settings = { rules: [] as DomainRule[], denylist: [] as string[] };
    const previewSnapshot = structuredClone(preview);
    const collection = buildRelatedDomainCandidateCollection(preview, settings);

    expect(collection.allCandidates.map((candidate) => candidate.category)).toEqual(["strong", "medium"]);

    const result = prepareSelectedRelatedDomainRules(
      settings,
      collection.allCandidates,
      new Set(["related.example"]),
      createdAt
    );
    ...
    expect(result).toMatchObject({
      ok: true,
      status: "added",
      addedRules: [{ domain: "related.example", includeSubdomains: true }]
    });
    expect(result.ok && result.addedRules).toHaveLength(1);
    expect(preview).toEqual(previewSnapshot);
    expect(settings).toEqual({ rules: [], denylist: [] });
  });
});
